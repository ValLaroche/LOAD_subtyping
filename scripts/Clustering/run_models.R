...
dirname = "UKBBN"
output = make_dataset(data_matrix = betas_UKBBN, metadata = pheno_UKBBN, dirname = dirname, 
                      db = "UKBBN", cpgs_intersect = cpgs_intersect)  
...
df_assignment4 = wrapper_models(data_matrix_AD, metadata_AD,cluster_vector = c(4,4), dirname = dirname)
df_assignment5 = wrapper_models(data_matrix_AD, metadata_AD,cluster_vector = c(5,5), dirname = dirname)
##### SPLSDA output of the identified subtypes ##### 
  rownames(df_assignment) = metadata_AD$Basename
  pheno_UKBBN = add_subtype_pheno(pheno_UKBBN, df_assignment)
...
##### Random permutations for specificity #####
  ...
  save(data_matrix_AD, metadata_AD, df_assignment, df_assignment4, df_assignment5, diablo_Methyls, df_shuffle, file = paste0("results/", dirname, "/res_clustering.Rdata"))
