...
##### Prepare data #####
  ...
  # 
  # output = gen_eigengenes(t(matrix_450k_BDR), 
  #                         t(matrix_450k_UKBBN), 
  #                         t(matrix_450k_PITT), 
  #                         t(data_matrix_AD_ROSMAP),
  #                         WGCNA_labels_450k)
  # 
  # BDR_eigen_450k = output[[1]]
  ...
  s 
##### WGCNA outputs #####
  trait_BDR = gen_subtype_matrix(WGCNA_phenoBDR, clustering = clustering, nsubtypes = 3)
  heatmap_module_to_subtype(eigengenes = BDR_eigen_EPIC, pheno_trait = trait_BDR, 
                            paste0("./WGCNA/", cohort, "/", clustering, "/corBDR_heatmap.pdf"))
  ...
  gen_anova_tukey_tests(pheno = WGCNA_phenoBDR, clustering = clustering, 
                        eigengenes = BDR_eigen_EPIC, filename = paste0("./WGCNA/", cohort, "/", clustering, "/anovatukeyB.pdf"))
  ...
  main_pheno = main_pheno[main_pheno$HC_clusters != "4",]
  main_expr_EPIC = main_expr_EPIC[,colnames(main_expr_EPIC) %in% main_pheno$Basename]
  main_expr_450k = main_expr_450k[,colnames(main_expr_450k) %in% main_pheno$Basename]
...
##### Subtypes #####
  ...
  # pred_mapping_1pred(data_disc = t(main_expr_EPIC), pheno_disc = main_pheno,
  #                    data_pred = t(disc_expr_EPIC_2), pheno_pred = pheno_disc_2, cohort = cohort, disc_cohort = pref_3,
  #                    clustering = clustering, filename = paste0("./WGCNA/", cohort, "/", clustering, "/mapping_to_", pref_3, "EPIC.pdf"))
  ...
  # for(subtype_hull in unique(main_pheno[[clustering]])){
  # tmp_fisher = test_chulls(data_disc = t(main_expr_EPIC), pheno_disc = main_pheno,
  #                          data_pred = t(disc_expr_EPIC_2), pheno_pred = pheno_disc_2, 
  #                          clustering = clustering, disc = pref_3)
  # df_all_tests_fisher = rbind(df_all_tests_fisher, tmp_fisher)
  # }
  for(subtype_hull in unique(main_pheno[[clustering]])){
...
##### Comparing clusters #####
  ...
  table(pheno_UKBBN$HC_clusters, pheno_UKBBN$KM_clusters)
  cor(pheno_UKBBN$HC_clusters, pheno_UKBBN$KM_clusters)
  
  table(pheno_PITT$HC_clusters, pheno_PITT$KM_clusters)
  cor(pheno_PITT$HC_clusters, pheno_PITT$KM_clusters)
  
  table(pheno_ROSMAP$HC_clusters, pheno_ROSMAP$KM_clusters)
  cor(pheno_ROSMAP$HC_clusters, pheno_ROSMAP$KM_clusters)
  
##### Fisher res #####
  
  res_fisher = read.table("WGCNA/Fisher_results_table.txt", sep = "\t", header = T)
  res_fisher$fillcolor = paste0(res_fisher$Subtype, res_fisher$Comparison)
  res_fisher$Subtype = factor(res_fisher$Subtype, levels = c("Red","Blue"))
  res_fisher[res_fisher$fillcolor %in% c("Redconf", "Blueconf"),]$fillcolor = "Conflict"
  res_fisher[res_fisher$Comparison == "conf",]$Comparison = "Conflict"
  res_fisher$Comparison = factor(res_fisher$Comparison, levels = c("Conflict",
                                                                   "BDR-UKBBN",
                                                                   "BDR-PITT",
                                                                   "BDR-ROSMAP",
                                                                   "UKBBN-PITT",
  ...
  res_fisher$fillcolor = factor(res_fisher$fillcolor, levels = c("Conflict",
                                                                 "RedBDR-UKBBN",
                                                                 "RedBDR-PITT",
                                                                 "RedBDR-ROSMAP",
                                                                 "RedUKBBN-PITT",
                                                                 ...
                                                                 "BlueUKBBN-ROSMAP",
                                                                 "BluePITT-ROSMAP"))
  res_fisher$Group = as.character(res_fisher$fillcolor)
  res_fisher[res_fisher$Group == "Conflict",]$Group = "Matching"
  
  
  ggplot(res_fisher, aes(x = Comparison, y = Value, fill = fillcolor)) +
    geom_bar(stat = "identity") +
    scale_fill_manual(values = c("RedBDR-UKBBN" = "#800020",
                                 "RedBDR-PITT" = "#800000",
                                 "RedBDR-ROSMAP" = "#960018",
                                 "RedUKBBN-PITT" = "#801818",
                                 "RedUKBBN-ROSMAP" = "#B22222",
                                 "RedPITT-ROSMAP" = "#C80815",
                                 "BlueBDR-UKBBN" = "#0A1172",
                                 "BlueBDR-PITT" = "#1338BE",
                                 "BlueBDR-ROSMAP" = "#1f456e",  
                                 "BlueUKBBN-PITT" = "#241571",
                                 "BlueUKBBN-ROSMAP" = "#151e3d",
                                 "BluePITT-ROSMAP" = "#051094",
                                 "Conflict" = "darkgreen"),
                      labels = c("Conflict",
                                 "BDR-UKBBN",
                                 "BDR-PITT",
                                 ...
                                 "UKBBN-ROSMAP",
                                 "PITT-ROSMAP")) +
    labs(fill = "Pairwise cohorts")+
    xlab("") + ylab("Significant matches") +
    theme_bw() +
