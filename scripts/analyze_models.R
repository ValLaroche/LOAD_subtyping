plot_model <- function(model, model_type, label_samples, outdir, numCC_clusters = 0){
  ...
  if(model_type == "iCB"){
    ...
    pdf(paste0(outdir, "VarExplained.pdf"),height=1000,width=1300)  
    showVarExplained(jive_model)  
    dev.off()
    
    pdf(paste0(outdir, "HeatmapsBRCA.pdf"),height=1000,width=1500)
    showHeatmaps(jive_model)
    dev.off()
    
    pdf(paste0(outdir, "JointPCA.pdf"),height=1000,width=1000)
    showPCA(jive_model, n_joint = 2)
    dev.off()
    
    pdf(paste0(outdir, "MorePCA.pdf"),height=1600,width=1600)
...
random_permutations <- function(data_matrix_AD, metadata_AD, diablo_Methyls){
  ...
  for(prop in c(seq(from = 1, to = 0.1, by = -0.1), seq(from = 0.09, to = 0.01, by = -0.01))){
    for(i in seq(1:10)){
      #Randomise outcomes
      to_random = sample(seq(1:nrow(metadata_AD)), size = prop*nrow(metadata_AD))
      ...
      diablo_shuffles[[1]] = mixOmics::splsda(t(data_matrix_AD), HC_shuffle, keepX=c(200,200,200,200,200,200), ncomp = 6)
      diablo_shuffles[[2]] = mixOmics::splsda(t(data_matrix_AD), KM_shuffle, keepX=c(200,200,200,200,200,200), ncomp = 6)
      diablo_shuffles[[3]] = mixOmics::splsda(t(data_matrix_AD), PAM_shuffle, keepX=c(200,200,200,200,200,200), ncomp = 6)
      diablo_shuffles[[4]] = mixOmics::splsda(t(data_matrix_AD), Spec_shuffle, keepX=c(200,200,200,200,200,200), ncomp = 6)
      #Calculate stats
