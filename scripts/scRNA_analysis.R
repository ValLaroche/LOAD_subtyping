...
load("scRNA/current_scRNA.Rdata")
...
for(i in names(tbl_RvC)){
  ...
  fisher_red_MIC = fisher.test(table_red_MICstate)
...
for(i in names(tbl_BvC)){
  ...
  fisher_blue_MIC = fisher.test(table_blue_MICstate)
...
table_res_melt = table_res_melt[!table_res_melt$variable %in% c("BLUE_sig", "RED_sid"),]
...
ggplot(table_res_melt, aes(x = cell_type, y = log_value, fill = variable)) +
  ...
  geom_text(aes(label = if_else(pvalue < 1e-5,"*","")), position = position_dodge(width = 0.9), vjust = -0.5, size = 8) +
